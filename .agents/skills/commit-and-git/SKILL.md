...
6. MUST: GitHub CLI で PR を作成・更新する場合は `gh pr create` / `gh api -X PATCH repos/{owner}/{repo}/pulls/{number}` を使用し、PR 本文を明示する; NEVER: Cursor の PR 管理ツール（`ManagePullRequest` 等）を使用する（`Open in Web` / `Open in Cursor` ボタンおよび `CURSOR_AGENT_PR_BODY_*` マーカーが本文に自動付与されるため）
