...
4. MUST: PR 作成は `gh pr create --body "<本文>"` または `gh pr create --body-file <file>` を使用する; NEVER: Cursor の PR 管理ツール（`ManagePullRequest` 等）で作成・更新しない（`Open in Web` / `Open in Cursor` ボタンおよび `CURSOR_AGENT_PR_BODY_*` マーカーが本文に自動付与されるため）
...
# PR 本文メンテナンス（Cursor ボタン除去）

1. IF: 過去 PR に `Open in Web` / `Open in Cursor` ボタンや `CURSOR_AGENT_PR_BODY_*` マーカーが付与されている; THEN MUST: ユーザー指示または skill-escalation 経由のクリーンアップ依頼時に除去する
2. MUST: 除去は GitHub REST API（`PATCH /repos/{owner}/{repo}/pulls/{number}`）または `GH_TOKEN` 付き `gh api -X PATCH repos/{owner}/{repo}/pulls/{number} --input -` を使用する; IF: `gh pr edit` が GraphQL スコープ不足で失敗する; THEN MUST: REST API に切り替える
3. MUST: 除去対象は `<div>...cursor.com/agents...</div>` ブロックおよび `CURSOR_AGENT_PR_BODY_BEGIN` / `CURSOR_AGENT_PR_BODY_END` HTML コメント
4. NEVER: 本文の実質的な説明部分を削除しない
