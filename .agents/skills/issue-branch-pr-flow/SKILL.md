...
1. IF: リポジトリが Vercel にデプロイされる; THEN MUST: 各 Vercel プロジェクトの Root Directory に対応する `vercel.json` に `templates/vercel-github-silent.json` と同等の設定を置く
2. MUST: `github.silent: true` を設定し、Vercel bot の PR / commit コメントを停止する
3. SHOULD: モノレポでは `apps/*` ごとに個別の `vercel.json` を配置する
4. IF: 過去 PR / Issue に `vercel[bot]` コメントが残っている; THEN MUST: ユーザー指示時に GitHub REST API（`DELETE /repos/{owner}/{repo}/issues/comments/{comment_id}`）で削除する
5. NOTE: PR ステータスチェック（`Vercel – <project>`）はプレビューデプロイに伴い残る。`deployment_status` イベントの activity log ノイズは Vercel Dashboard → Project Settings → Git で `Pull Request Comments` / `deployment_status` Events を OFF にする（`vercel.json` では無効化不可）
