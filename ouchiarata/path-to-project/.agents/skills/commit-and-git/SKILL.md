...
3. MUST: `scope` にはリポジトリ全体を指す場合の `repo`、または実際に存在するフォルダ／パッケージのパスのみを使用する; NEVER: `workspace` など実体のない総称や、存在しないフォルダ名を `scope` に使用しない（NG: `feat(workspace): ...` → OK: `feat(repo): ...` / `feat(ouchiarata/path-to-project): ...` / `feat(packages/log): ...`）
4. MUST: 論理的なスコープ（パッケージ、機能）ごとにコミットを分割する; NEVER: 無関係な変更をまとめてコミットしない
5. 変更をコミットする際:
...
6. NEVER: `Co-Authored-By` を追加しない
7. NEVER: `git add .` や `git add -A` を使用しない
8. NEVER: 無関係な変更を1つのコミットに混在させない
