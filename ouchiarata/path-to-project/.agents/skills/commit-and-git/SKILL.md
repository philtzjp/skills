...
3. MUST: `scope` にはリポジトリ全体を指す場合の `repo`、または実際に存在するフォルダ／パッケージのパスのみを使用する; MUST: 隠しディレクトリは先頭ドットを含めて書く（`agents` ではなく `.agents`）; NEVER: `workspace` など実体のない総称や、存在しないフォルダ名を `scope` に使用しない（NG: `feat(workspace): ...` / `feat(design): ...` / `feat(agents): ...` → OK: `feat(repo): ...` / `feat(.agents): ...` / `feat(ouchiarata/path-to-project): ...` / `feat(packages/log): ...`）
4. MUST: 説明は動作で終える（OK: `〜する` / `〜追加` / `〜修正` / `〜削除` / `〜実装` など）; NEVER: 動作を伴わない名詞で終える（NG: `feat(api): ユーザー認証` → OK: `feat(api): ユーザー認証を追加`）; NEVER: 説明に emoji を含めない
5. MUST: 論理的なスコープ（パッケージ、機能）ごとにコミットを分割する; NEVER: 無関係な変更をまとめてコミットしない
6. 変更をコミットする際:
...
7. NEVER: `Co-Authored-By` を追加しない
8. NEVER: `git add .` や `git add -A` を使用しない
9. NEVER: 無関係な変更を1つのコミットに混在させない
