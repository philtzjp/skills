...
9. NEVER: squash merge / squash commit を使用しない。ただしユーザーが明示的に指示した場合のみ例外とする
10. NEVER: ユーザーの確認なしにブランチを切り替えない
11. NEVER: ユーザーの承認なしに `git pull` や `git rebase` を実行しない
12. NEVER: ユーザーの確認なしにローカルブランチを削除しない
